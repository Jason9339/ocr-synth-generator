...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --out_dir_h OUT_DIR_H         Horizontal output directory (default: output/ocr_out_h)"
            echo "  --out_dir_v OUT_DIR_V         Vertical output directory (default: output/ocr_out_v)"
...
echo "     python3 convert_to_lmdb.py --src output/ocr_out_h --dst out_h.lmdb"
echo "     python3 convert_to_lmdb.py --src output/ocr_out_v --dst out_v.lmdb"
echo ""
echo "  2. Transfer LMDB files to NFS (if needed):"
echo "     rsync -a out_h.lmdb out_v.lmdb /mnt/whliao/lmdb/"
echo ""
echo "  3. (Optional) Clean up images:"
echo "     rm -rf output/ocr_out_h output/ocr_out_v"
echo ""
